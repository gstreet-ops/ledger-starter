...
export async function addAccount(formData: FormData) {
  ...
  const gaFormCategory = (formData.get("gaFormCategory") as string) || undefined;
  ...
  try {
    await createAccount({ code, name, type, scheduleCLine, gaFormCategory });
...
export async function editAccount(formData: FormData) {
  ...
  const gaFormCategory = (formData.get("gaFormCategory") as string) || null;

  try {
    await updateAccount(id, { name, scheduleCLine, gaFormCategory });
    revalidatePath("/accounts");
    return { success: true };
  } catch (e: any) {
